...
Este repositório contém o código-fonte de uma API REST de Calculadora desenvolvida em Elixir utilizando a biblioteca Plug e o servidor Cowboy. O gerenciador Mix (mix.exs) controla as dependências e a compilação. O processo de conteinerização exige o download de pacotes (mix deps.get) e a geração de uma release (mix release), que empacota o código compilado com a máquina virtual Erlang (BEAM). A aplicação expõe a porta 8080.
